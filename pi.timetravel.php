  ...
  */
  var $url_formats = array(
	...
	*/
    ...
    $this->by = $TMPL->fetch_param('by') ? $TMPL->fetch_param('by') : $this->by;
  ...
  function _parse_template()
  {
    ...
    {				
    	...
    	{				
    		$tagdata = str_replace($matches[0][$j], $LOC->decode_date($matches[2][$j], $this->current), $this->tagdata);
    	}
    } else {
    ...
    foreach ($TMPL->var_pair as $key => $val)
    {
      ...
      $inner = $TMPL->fetch_data_between_var_pairs($tagdata, $key);
      ...
      $tagdata = preg_replace("/".LD.$key.RD."(.*?)".LD.SLASH.$key.RD."/s", $inner, $tagdata);
  ...
  function _is_single_entry_page()
  {
    ...
    $in = $IN->QSTR;
 	  ...
 	  $results = $DB->query("SELECT entry_id, url_title FROM exp_weblog_titles WHERE entry_id = '$in' OR url_title = '$in'");
  ...
  function _set_current_period()
  {
    ...
    {
      case 'day':
        preg_match('/(?P<year>\d{4})\/(?P<month>\d{2})\/(?P<day>\d{2})/', $IN->QSTR, $parts);
        ...
        break;
      case 'month':
        preg_match('/(?P<year>\d{4})\/(?P<month>\d{2})/', $IN->QSTR, $parts);
        ...
        break;
      case 'year':
        preg_match('/(?P<year>\d{4})/', $IN->QSTR, $parts);
        if( isset($parts['year']))
        {
...
If you are using Timetravel to wakl through years, you need to add year='{segment_n}' and dynamic='off' to your weblog:entries tag.
...
class Timetravel_path
{
  ...
  function alter_path($matches) 
  {
    ...
    $repl = $FNS->create_url(implode('/', array_merge($path, $time)));
    $return = str_replace($matches[0], $repl, $matches[0]);    
